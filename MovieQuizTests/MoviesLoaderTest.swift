...
//  MovieQuizTests
//
//  Created by Owi Lover on 7/19/24.
//
...
class MoviesLoaderTest: XCTestCase {
    func testMoviesDidLoad() throws {
        
        let stubNetworkClient = StubNetworkClient(emulateError: false)
        let loader = MoviesLoader(networkClient: stubNetworkClient)
        
        let expectation = expectation(description: "Movies did load!")
        
        loader.loadMovies() { result in
            switch result {
            case .success(let movies):
                XCTAssertEqual(movies.moviesArray.count, 2)
                expectation.fulfill()
            case .failure(_):
                XCTFail("Movies didn't load!")
            }
        }
        waitForExpectations(timeout: 2.0)
    }
    
    func testMoviesDidNotLoad() throws {
        
        let stubNetworkClient = StubNetworkClient(emulateError: true)
        
        let loader = MoviesLoader(networkClient: stubNetworkClient)
        
        let expectation = expectation(description: "Movies didn't load!")
        
        loader.loadMovies() { result in
            switch result {
            case .success(_):
                XCTFail("Movies did load, how strange!")
